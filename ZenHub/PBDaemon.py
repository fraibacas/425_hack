...
EVENT_FLOW_NORMAL = 1
EVENT_FLOW_THROTTLE = 2
EVENT_FLOW_PAUSE = 3

EVENT_FLOWS = {
    1:'EVENT_FLOW_NORMAL',
    2:'EVENT_FLOW_THROTTLE',
    3:'EVENT_FLOW_PAUSE'
}

EVENT_FLOWS_MAP = collections.defaultdict(lambda: 'Unknown', EVENT_FLOWS)
...
class EventQueueManager(object):
    ...
    def _initQueues(self):
        ...
        if not self._pauseEvents:
            # if events paused, don't replace perf and event queues
    ...
    def sendEvents(self, event_sender_fn):
        ...
        if not paused:
            ...
            prev_perf_event_queue = collections.deque()
            prev_event_queue = collections.deque()
        self._initQueues()
    ...
    def pauseEvents(self, pauseEvents=True):
        """
        pause sending events. Heartbeat events will still be sent
        """
        self._pauseEvents = pauseEvents
...
class PBDaemon(ZenDaemon, pb.Referenceable):
    ...
    def remote_setEventFlow(self, flow):
        if self._eventFlow != flow:
            self.log.info("Setting Event Flow Events %s", flow)
        self._eventFlow = flow
        if flow == EVENT_FLOW_PAUSE:
            self.eventQueueManager.pauseEvents(True)
        else:
            self.eventQueueManager.pauseEvents(False)

    def getEventFlow(self):
        return self._eventFlow
