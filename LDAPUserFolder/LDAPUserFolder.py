...
class LDAPUserFolder(BasicUserFolder):
    ...
    def _lookupuserbyattr(self, name, value, pwd=None):
        ...
        if name == 'dn':
            if value.find(',') == -1:
                ...
                logger.debug(msg)
                logger.info("login failed: %s (not a valid DN)", value)
        ...
        if res['size'] == 0 or res['exception']:
            ...
            logger.debug(msg)
            logger.info("login failed: %s (no such user)", value)
            if res['exception']:
                raise Exception("LDAP Error: %s" % res['exception'])
            return None, None, None, None
        ...
        if pwd is not None:
            ...
            if self._binduid_usage != 1:
                ...
                try:
                    ...
                    logger.debug(msg)
                    logger.info("login failed: %s (invalid password)", value)
                    return None, None, None, None
            ...
            if auth_res['size'] == 0 or auth_res['exception']:
                ...
                logger.debug(msg)
                logger.info("login failed: %s (lookup failed)", value)
                if auth_res['exception']:
                    raise Exception("LDAP Error: %s" % auth_res['exception'])
                return None, None, None, None
        ...
        logger.info("login successful: %s", value)
    ...
    def manage_edit( self, title, login_attr, uid_attr, users_base
                   ...
                   , extra_user_filter='', extra_group_filter='', REQUEST=None
                   ):
        ...
        self._extra_group_filter = extra_group_filter.strip()
    ...
    def manage_addServer( self
        ...
        self._delegate.addServer(host, port, use_ssl, conn_timeout, op_timeout, self_signed_cert)
    ...
    def getUserByAttr(self, name, value, pwd=None, cache=0, session_data=None):
        """
        ...
        negative_cache_key = '%s:%s:%s:%s' % (aq_parent(self).id, name, value, sha_new(pwd or '').hexdigest())
        plugin_id = self.getParentNode().id

        if session_data is not None:
            if not session_data.has_key("ldap_negative_cache"):
                logger.debug("{plugin_id} initializing session ldap_negative_cache for all plugins".format(**locals()))
                session_data.set("ldap_negative_cache", {})
            if plugin_id not in session_data.get("ldap_negative_cache"):
                logger.debug("{plugin_id} initializing session ldap_negative_cache for {plugin_id}".format(**locals()))
                session_data.get("ldap_negative_cache")[plugin_id] = {}

        def sesneg():
            return session_data.get("ldap_negative_cache")[plugin_id]

        if cache:

            if session_data is not None:
                session_user = sesneg().get(negative_cache_key)
                if session_user:
                    if self._cache("negative").isUserFresh(session_user):
                        local_user = self._cache("negative").get(negative_cache_key)
                        if local_user is None or local_user.getCreationTime() < session_user.getCreationTime():
                            self._cache("negative").set(negative_cache_key, session_user)
                        elif session_user.getCreationTime() < local_user.getCreationTime():
                            logger.debug("{plugin_id} copying {negative_cache_key} to session ldap_negative_cache".format(**locals()))
                            sesneg()[negative_cache_key] = local_user
                    else:
                        logger.debug("{plugin_id} deleting {negative_cache_key} from session ldap_negative_cache".format(**locals()))
                        del sesneg()[negative_cache_key]

            if self._cache('negative').get(negative_cache_key) is not None:
                logger.debug("{plugin_id} negative cache contains key '{negative_cache_key}'".format(**locals()))
                return None

            cached_user = self._cache(cache_type).get(value, pwd)

            if cached_user:
                if getattr(cached_user, 'invalid', None):
                    logger.debug("{plugin_id} {cache_type} cache contains key '{value}', but cached user is invalid".format(**locals()))
                    return None

                logger.debug("{plugin_id} {cache_type} cache contains key '{value}'".format(**locals()))
                return cached_user
                
            else:
                logger.debug("{plugin_id} {cache_type} cache does NOT contain key '{value}'".format(**locals()))

        user_roles, user_dn, user_attrs, ldap_groups = None, None, None, None
        try:
            user_roles, user_dn, user_attrs, ldap_groups = self._lookupuserbyattr(
                name=name, value=value, pwd=pwd
                )
        except Exception as e:
            session_data['login_message'] = str(e)
            
        if user_dn is None:
            logger.debug('getUserByAttr: "%s=%s" not found' % (name, value))
            user = NonexistingUser()
            self._cache('negative').set(negative_cache_key, user)
            if session_data is not None:
                logger.debug("{plugin_id} adding {negative_cache_key} to session ldap_negative_cache".format(**locals()))
                sesneg()[negative_cache_key] = user
            return None
    ...
    def getUser(self, name, pwd=None, session_data=None):
        ...
        user = self.getUserByAttr(self._login_attr, name, pwd, cache=1, session_data=session_data)
    ...
    def searchUsers(self, attrs=(), exact_match=True, **kw):
    ...
    def searchGroups(self, attrs=(), exact_match=True, **kw):
    ...
    def findUser(self, search_param, search_term, attrs=(), exact_match=True):
    ...
    def getGroups(self, dn='*', attr=None, pwd=''):
        ...
        if self._local_groups:
            ...
            if dn != '*':
                ...
                if self._extra_group_filter:
                    group_filter = '(&{0}(|'.format(self._extra_group_filter)
                else:
                    group_filter = '(|'
