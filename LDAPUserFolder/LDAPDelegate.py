...
class LDAPDelegate(Persistent):
    ...
    def addServer( self
        ...
        for server in self._servers:
            if ( str(server['host']) == str(host) and
                ...
                server['self_signed_cert'] = self_signed_cert
                break
    ...
    def connect(self, bind_dn='', bind_pwd=''):
        ...
        invalidCache = None
        invalidCredentialsE = None
        conn = getResource('%s-connection' % self._hash, str, ())
        # Need to account for the following three things in this method
        # 1. Do not incement the invalid password more than once on invalid credentials
        # 2. When the cached connection is expired that we establish a new one (this is done with the invalidCache var)
        # 3. Do not let users log in with a bad password (this is done with the invalidCredentialsE)
        if not isinstance(conn._target, str):
            try:
                logger.debug("connect->simple_bind_s uri='{0}', user_dn='{1}'".format(conn._uri, user_dn))
                ...
                # we can get a server down when the server is really because the cached connection
                # was disconnected
                logger.info("LDAP Connection time out reestablishing...")
                pass
        ...
        for server in self._servers:
            ...
            # to avoid incrementing badPwdCount twice on the LDAP server and
            # increasing the chance that the user will be locked out don't
            # call _connect (and therefore simple_bind_s again) if this is the
            # same server that was retrieved with getResource earlier in this
            # connect method
            if (not isinstance(conn._target, str) and conn._uri == conn_string) and invalidCache is None:
                if invalidCredentialsE:
                    e = invalidCredentialsE
                continue

            if server['protocol'] == 'ldaps':
                certfile = os.path.join(os.environ['ZENHOME'], 'etc',
                    "%s.pem" % server['host'])
                if os.path.exists(certfile):
                    ldap.set_option(ldap.OPT_X_TLS_CACERTFILE, certfile)
            
            if server['self_signed_cert']:
    ...
    def _connect( self
        ...
        logger.debug("_connect->simple_bind_s uri='{0}', user_dn='{1}', user_pwd='{2}'".format(connection._uri, user_dn, user_pwd))
