...
class JobManager(ZenModelRM):
    ...
    def deleteUntil(self, untiltime):
        """
        Delete all jobs older than untiltime (type: datetime.timedelta).
        """
        now = datetime.now()
        count = 0
        log.info('Deleting jobs older than %s days...', untiltime.days)
        for b in self.getCatalog()():
            o = b.getObject()
            j_description = o.description
            age = now - o.date_done
            delta = age - untiltime
            if delta.days > 0:
                try:
                    self.deleteJob(o.getId())
                    count += 1
                    log.info("Deleted job: %s", j_description)
                except Exception as ex:
                    log.warning("Error while deleting the job: %s - %s", j_description, ex)
        log.info("Deleted %s old jobs.", count)        
