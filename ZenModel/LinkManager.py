...
class DrawNetwork(object):
    """ Class to store zDrawMapLinks for all networks """

    def __init__(self):
        self.draw_net = OOBTree()

    def add_net(self, network_id, zDrawMapLinks=True):
        self.draw_net.update({network_id : zDrawMapLinks})
        
    def remove_net(self, network_id):
        if network_id in self.draw_net:
            del self.draw_net[network_id]

    def update_net(self, network_id, value):
        self.draw_net.update({network_id : value})

    def getNetworks(self):
        return self.draw_net
...
class LinkManager(Folder):
    ...
    def getChildLinks(self, organizer):
        ...
        draw_nets = self.dmd.ZenLinkManager.networks.getNetworks()

        for link in links:
            if not draw_nets.get(link.networkId):
                continue
            dev = _whichdev(link)
            org = _whichorg(link)
