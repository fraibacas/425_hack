...
class IpNetwork(DeviceOrganizer):
    ...
    def setZenProperty(self, zProperty, value):
        if zProperty == 'zDrawMapLinks':
            net = self.getPrimaryUrlPath()
            self.dmd.ZenLinkManager.networks.update_net(net, value)
            subnets = self.getSubNetworks()
            for subnet in subnets:
                if not subnet.hasProperty('zDrawMapLinks'):
                    snet = subnet.getPrimaryUrlPath()
                    self.dmd.ZenLinkManager.networks.update_net(snet, value)

        return super(IpNetwork,self).setZenProperty(zProperty, value)

    def deleteZenProperty(self, zProperty):
        if zProperty == 'zDrawMapLinks':
            parentProp = self.getParentNode().zDrawMapLinks
            self.dmd.ZenLinkManager.networks.update_net(self.getPrimaryUrlPath(), parentProp)
            for subnet in self.getSubNetworks():
                if not subnet.hasProperty('zDrawMapLinks'):
                    self.dmd.ZenLinkManager.networks.update_net(subnet.getPrimaryUrlPath(), parentProp)

        super(IpNetwork,self).deleteZenProperty(zProperty)
    ...
    def createNet(self, netip, netmask=24):
        ...
        net = netobj.getPrimaryUrlPath()
        self.dmd.ZenLinkManager.networks.add_net(net)
