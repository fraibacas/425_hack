...
class MaintenanceWindow(ZenModelRM):
    ...
    def occurDay(self, start, now, skip=1, day=6, occur=0):
        ...
        log.debug('start date: %s; day: %d; occur: %d; skip: %d', str(date), day, occur, skip)
        ...
        for mday, wday in flatter:
            if wday == day and mday > 0:
                ...
                if count == tmp_occur + 1 and mday >= date.tm_mday:
                    ...
                    startTime = time.mktime((date.tm_year, date.tm_mon, mday) + time_for_mktime)
                    # do we need to skip this day?
                    if skip > 1:
                        log.debug('skipping this occurrence. skip = %d', skip)
                        return self.occurDay(startTime + DAY_SECONDS, now, skip - 1, day, tmp_occur)
                    elif startTime >= now:
                        log.debug('Window will start on: %s', str(time.localtime(startTime)))
                        return startTime
