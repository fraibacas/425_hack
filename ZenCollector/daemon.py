...
class CollectorDaemon(RRDDaemon):
    ...
    def _updateConfig(self, cfg):
        ...
        # nextExpectedRuns is used to capture the next run time of the old tasks so that
        # if there's a new task having the same config ID, then we can assign the new task
        # to run at the time when the old task would've run.
        nextExpectedRuns = {}
        ...
        for (taskName, task_) in newTasks.iteritems():
            ...
            nextExpectedRun = nextExpectedRuns.get(taskName)
            if nextExpectedRun:
