...
from Products.ZenEvents.ZenEventClasses import Cmd_Fail, Error, Clear
    ...
    def doTask(self):
        ...
        try:
            ...
            collectionErrorEvent = {'device':self.deviceId, 'severity':Error, 'eventClass':Cmd_Fail,
                                    ...
                                    'eventKey':'collectionError'}
            errorEventId = str(self.deviceId)+"_"+str(self.component)
            self.daemon.errorEvents.add(errorEventId)
            ...
            if results:
                ...
                clearEventId = str(self.deviceId)+"_"+str(self.component)
                if clearEventId in self.daemon.errorEvents:
                    self.daemon.errorEvents.remove(clearEventId)
                    collectionClearEvent = {'device':self.deviceId, 'severity':Clear, 'eventClass':Cmd_Fail,
                                            'summary':'Collecting got back to normal',
                                            'component':self.component, 'agent':self.component,
                                            'eventKey':'collectionError'}
                    yield self.outputProxy.sendEvent(collectionClearEvent)
