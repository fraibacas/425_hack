...
class DeviceAddView(BrowserView):
    ...
    def autodiscovery(self):
        ...
        try:
            self.context.dmd.ZenPackManager.packs._getOb('ZenPacks.zenoss.Microsoft.Windows')
            zProperties['zWinRMUser'] = self.request.form.get('winusername')
            zProperties['zWinRMPassword'] = self.request.form.get('winpass')
        except AttributeError:
            zProperties['zWinUser'] = self.request.form.get('winusername')
            zProperties['zWinPassword'] = self.request.form.get('winpass')
