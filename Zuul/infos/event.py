...
class EventCompatInfo(object):
    ...
    @property
    def component(self):
        element_sub_uuid = self._eventActor.get('element_sub_uuid')
        data = {
            'text': self._eventActor.get('element_sub_title'),
            'uid': self._getPathFromUuid(element_sub_uuid),
            'uuid': element_sub_uuid
        }
        # Assume True if 'valid_element_uuid' doesn't exist.
        # Should only be False if the uuid is known to be invalid.
        valid = self._eventActor.get('valid_element_sub_uuid', True)
        # Set url to None if the uuid is not valid.
        # A null url signals the UI to not link the device.
        data['url'] = self._uuidUrl(element_sub_uuid) if valid else None
        return data
    ...
    def device(self):
        ...
        if device_url is None:
            element_uuid = self._eventActor.get('element_uuid')
            data = dict(
                text=self._eventActor.get('element_title'),
                uid=self._getPathFromUuid(element_uuid),
                uuid=element_uuid
            )
            # Assume True if 'valid_element_uuid' doesn't exist.
            # Should only be False if the uuid is known to be invalid.
            valid = self._eventActor.get('valid_element_uuid', True)
            # Set url to None if the uuid is not valid.
            # A null url signals the UI to not link the device.
            data['url'] = self._uuidUrl(element_uuid) if valid else None
            return data
        else:
