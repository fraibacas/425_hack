...
class EventsRouter(DirectRouter):
    ...
    def _markInvalidUuids(self, events):
        """
        ...
        for event_summary in events:
            ...
            for key in ("element_uuid", "element_sub_uuid"):
                value = actor.get(key)
                if value:
                    validKey = "valid_%s" % key
                    actor[validKey] = bool(value in self.manager.table)
            yield event_summary

    def getAllManagedObj(self):
        user = self.context.dmd.ZenUsers.getUserSettings()
        return [IGlobalIdentifier(ar.managedObject()).getGUID() for ar in user.getAllAdminRoles()\
                if ar.managedObject() is not None and isinstance(ar.managedObject(), (Device, DeviceClass, DeviceGroup, System, Location))]
                
    def removeNotValidTags(self, params):
        tags = []
        user = self.context.dmd.ZenUsers.getUserSettings()
        for tag in params:
            obj = self.manager.getObject(tag)
            if Zuul.checkPermission(ZEN_MANAGE_EVENTS, obj): 
                tags.append(tag)
        if not tags:
            tags = self.getAllManagedObj()
        return tags
    ...
    def queryArchive(self, page=None, limit=0, start=0, sort='lastTime', dir='desc', params=None, exclusion_filter=None, keys=None, uid=None, detailFormat=False):
        ...
        if user.hasNoGlobalRoles():
            if params.get('tags') == None: 
                params['tags'] = self.getAllManagedObj()
            else: 
                params['tags'] = self.removeNotValidTags(params['tags'])
        ...
        evdata = self._markInvalidUuids(events['events'])
