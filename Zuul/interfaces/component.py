...
class IIpInterfaceInfo(IComponentInfo):
    ...
    status = schema.TextLine(
        ...
        description=_t(u"Are there any active status events for this component?"
...
class IOSProcessInfo(IComponentInfo):
    ...
    processClass = schema.Entity(title=_t(u"Process Class"), group="Overview",
                                 order=1)
    getMonitoredProcesses = schema.TextLine(title=_t(u"Process(es) identified by the most recent model"), group="Details", 
    ...
    minProcessCount = schema.Int(title=_t(u"Min Process Count"),
                                 group="Details", alwaysEditable=True)
    maxProcessCount = schema.Int(title=_t(u"Max Process Count"),
                                 group="Details", alwaysEditable=True)
