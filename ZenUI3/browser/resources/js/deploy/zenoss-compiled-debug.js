...
(function(){
    ...
    var vtypes = {
        ...
        linkedDateField: function(val, field) {
            var date = field.parseDate(val),
                f = field.up().getForm();
            if (field.name === "StartDate" &&
            (!this.start_date || date.getTime()!=this.start_date.getTime()) ) {
                var endDateField = f.findField('EndDate');
                this.start_date = date;
                endDateField.setMinValue(date)
                endDateField.validate();
            } else if (field.name === "EndDate" &&
            (!this.end_date || date.getTime()!= this.end_date.getTime())) {
                var startDateField = f.findField('StartDate');
                this.end_date = date;
                startDateField.setMaxValue(date)
                startDateField.validate();
            }
            return true;
        },
        linkedTimeField: function(val, field) {
            var f = field.up().getForm()
                time = field.parseDate(val),
                etf = f.findField('EndTime'),
                stf = f.findField('StartTime'),
                edv = field.up().up().getForm().findField('EndDate').getSubmitValue(),
                sdv = field.up().up().getForm().findField('StartDate').getSubmitValue()
            if (edv && edv !== sdv) {
                stf.setMaxValue(0)
                etf.setMinValue(0)
            } else {
                if (field.name === "StartTime") {
                    etf.setMinValue(time)
                }  else if (field.name === "EndTime") {
                    stf.setMaxValue(time)
                }
            }
            return true;
        }
                ...
                '->',
                 {
                    ...
                    handler: function (btn) {
                        selectCustomDateRange(btn,Ext.getCmp(cardid))
                        }
                },'-', {
                    xtype: 'button',
                    ref: '../resetBtn',
        ...
        config = Ext.applyIf(config||{}, {
            ...
            columns: [{
                ...
                renderer: function(ips) {
                    return Ext.isArray(ips) ? ips.join(', ') : ips.toString();
                }
            },{
                id: 'description',
                dataIndex: 'description',
