...
class ZenBackup(ZenBackupBase):
    ...
    def buildOptions(self):
        ...
        self.parser.add_option('--mibs',
                               dest="backupMibs",
                               default=False,
                               action='store_true',
                               help='include mibs to the backup')
        self.parser.add_option('--libexec',
                               dest="backupLibexec",
                               default=False,
                               action='store_true',
                               help='include scripts to the backup')
    ...
    def doBackup(self):
        ...
        if self.options.backupMibs:
            self._backup.backupMibs()
        else:
            self.log.info('Skipping backup MIBs.')
    
        if self.options.backupLibexec:
            self._backup.backupLibexec()
        else:
            self.log.info('Skipping backup $ZENHOME/libexec.')
...
class CoreBackup(object):
    ...
    def backupZodb(self):
        ...
        self._backupMySqlDb(self.options.zodb_host, self.options.zodb_port,
                            ...
                            socket=self.options.zodb_socket, ignore_tables=['connection_info'])
    ...
    def backupMibs(self):
        ...
        @return:
        """
        mibDir = zenPath('share', 'mibs')
        if os.path.isdir(mibDir):
            # Copy MIB files to backup dir
            self.log.info('Backing up %s' % mibDir)
            etcTar = tarfile.open(os.path.join(self.tempDir, 'mibs.tar'), 'w')
            etcTar.dereference = True
            etcTar.add(mibDir, 'mibs')
            etcTar.close()
            self.log.info("Backup of %s completed", mibDir)
        else:
            self.log.info('%s does not exist', mibDir)
    ...
    def backupLibexec(self):
        ...
        @return:
        """
        libexecDir = zenPath('libexec')
        if os.path.isdir(libexecDir):
            # Copy scripts to backup dir
            self.log.info('Backing up %s', libexecDir)
            etcTar = tarfile.open(os.path.join(self.tempDir, 'libexec.tar'), 'w')
            etcTar.dereference = False
            etcTar.add(zenPath('libexec'), 'libexec')
            etcTar.close()
            self.log.info("Backup of %s completed", libexecDir)
        else:
            self.log.info('%s does not exist', libexecDir)
    ...
    def _backupMySqlDb(self, host, port, db, user, passwdType, sqlFile, socket=None, tables=None, ignore_tables=None):
        ...
        if ignore_tables:
            for table in ignore_tables:
                command.append('--ignore-table=%s.%s' % (db, table))
...
class EnterpriseBackup(CoreBackup):
    ...
    def __init__(self, workingDir, options):
        ...
        if (self._zepHost == self._zodbHost) and (self.options.backupZopeDb != self.options.backupEventsDb):
            self.log.error(
                "Cannot exclude zep or zodb from backup when both are "
                "running on the same instance and using enterprise backup "
                "option"
            )
            raise ZenBackupException(
                "Cannot exclude zep or zodb when using enterprise backup",
                True
            )
    ...
    def backupZodb(self):
        ...
        """
        self.log.info('Backing up the ZODB.')
        # Verify that ZODB database can be backed up by enterprise tools.
        credential = getPassArg(self.options, "zodb_password")
        mysql_cmnd = getMySQLCommand(
            self.options.zodb_host, self.options.zodb_port,
            self.options.zodb_user, credential,
            socket=self.options.zodb_socket
        )
        sql = (
            "SELECT COUNT(1) FROM information_schema.tables "
            "WHERE table_schema = 'zodb' AND table_name = 'object_state' "
                "AND row_format = 'compressed';"
        )
        mysql_cmnd.extend(["-e", sql])
        (output, warnings, rc) = runCommand(mysql_cmnd)
        if output.strip() == '1':
            raise ZenBackupException(
                "Cannot create database backup using --interprise-tools "
                "when the object_state table in the zodb database has "
                "a row_format of 'compressed'.  Please alter it to "
                "'compact' to create a backup.", True
            )
    ...
    def _eBackupMySqlDb(self, mbiFile, host, port):
        ...
        with gzip.open(os.path.join(self.tempDir, mbiFile), 'wb') as gf:
            ...
            if meb_rc:
                ...
                #       --defaults-file=/opt/zends/etc/zends.cnf
                #       --user=root --no-locking
                #       --backup-dir=/tmp/meb --backup-image=-
                #       --with-timestamp backup-to-image | gzip > blam.gz

    @property
    def _opts(self):
        return ["--user=root", "--no-locking", "--backup-image=-", "--with-timestamp",
                "--exclude-tables=\(connection_info\|zodb_session\..*\)"]
...
if __name__ == '__main__':
    ...
    try:
        if zb.doBackup():
            sys.exit(-1)
    except ZenBackupException as ex:
        zb.log.error(ex)
        sys.exit(1)
