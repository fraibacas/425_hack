...
from Products.ZenUtils.ZenBackupBase import (
    ZenBackupBase, getMySQLCommand, runCommand, is_db_remote,
    getEnterpriseToolPath, getPassArg,
    PIPE, BACKUP_DIR, CONFIG_FILE, CONFIG_SECTION
)
...
class ZenRestore(ZenBackupBase):
    ...
    def buildOptions(self):
        ...
        self.parser.add_option('--deletePreviousMibs',
                               dest="deletePreviousMibs",
                               default=False,
                               action='store_true',
                               help='Delete ALL existing MIBs before restoring?')
        self.parser.add_option('--deletePreviousLibexec',
                               dest="deletePreviousLibexec",
                               default=False,
                               action='store_true',
                               help='Delete ALL existing scripts in $ZENHOME/libexec before restoring?')
    ...
    def restoreMibs(self):
        if self.options.deletePreviousMibs:
            cmd = 'rm -rf %s' % os.path.join(zenPath(), 'share', 'mibs')
            if os.system(cmd):
                return -1
        if os.path.isfile(os.path.join(self.tempDir, 'mibs.tar')):
            self.msg('Restoring MIBs.')
            cmd = 'tar Cxf %s %s' % (
                zenPath(),
                os.path.join(self.tempDir, 'mibs.tar'))
            if os.system(cmd):
                return -1
        else:
            self.msg("This backup does not contain MIBs to restore.")
            
    def restoreLibexec(self):
        if self.options.deletePreviousLibexec:
            cmd = 'rm -rf %s' % os.path.join(zenPath(), 'libexec')
            if os.system(cmd):
                return -1
        if os.path.isfile(os.path.join(self.tempDir, 'libexec.tar')):
            self.msg('Restoring $ZENHOME/libexec.')
            cmd = 'tar Cxf %s %s' % (
                zenPath(),
                os.path.join(self.tempDir, 'libexec.tar'))
            if os.system(cmd):
                return -1
        else:
            self.msg("This backup does not contain custom scripts to restore.")
    ...
    def doRestore(self):
        ...
        restoreFactory = BackupRestoreFactory()
...
class BackupRestoreFactory(object):
    """
...
class CoreBackupRestore(object):
    ...
    def restoreZODBSessionSQL(self):
        """Ensures that a zodb_session database exists.
        """
        self.msg('Restoring ZODB session database.')
        self.restoreMySqlDb(self.options.zodb_host, self.options.zodb_port,
                            self.options.zodb_db + "_session",
                            self.options.zodb_user,
                            getPassArg(self.options, 'zodb_password'),
                            sqlFile=None, socket=self.options.zodb_socket)
        self.msg('Done Restoring ZODB session database.')

    def restoreMySqlDb(self, host, port, db, user, passwd, sqlFile=None, socket=None):
        """
        Recreates the specified database and optionally runs a SQL file.
        """
        mysql_cmd = getMySQLCommand(
            host, port, user, passwd,
            compress=self.options.compressTransport, socket=socket
        )
        sql = "CREATE DATABASE IF NOT EXISTS `%s`" % db
        create_db = list(mysql_cmd) + ["-e", sql]
        (output, warnings, rc) = runCommand(create_db)
        ...
        # Convert mysql_cmd from a list into a string, necessary
        # for the .format() call below.
