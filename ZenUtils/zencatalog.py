...
# Container for input script arguments that should be removed from sys.argv
ARG_DROP_LIST = []
...
def drop_arguments(drop_list=None):
    if drop_list is None:
        drop_list = ARG_DROP_LIST

    sys_argv_iter = iter(sys.argv[1:])
    sys.argv = sys.argv[:1]

    def rebuild_sys_argv(argument):
        if argument.startswith('-'):
            argument, value = unpack_argument(argument)
            if argument not in drop_list:
                sys.argv.append(argument)
                next_argument = value or sys_argv_iter.next()
                if not next_argument.startswith('-'):
                    sys.argv.append(next_argument)
                    rebuild_sys_argv(sys_argv_iter.next())
                else:
                    rebuild_sys_argv(next_argument)
        rebuild_sys_argv(sys_argv_iter.next())

    def unpack_argument(argument):
        value = None
        delimiter = argument.rfind('=')
        if delimiter != -1:
            argument, value = argument[:delimiter].strip(), argument[delimiter+1:].strip()
...
class ZenCatalogBase(ZenDaemon):
    ...
    def buildOptions(self):
        """
            ...
            IMPORTANT: regarding to implementation details, ARG_DROP_LIST should be filled in 
        ...
        ARG_DROP_LIST.append("--createcatalog")
    ...
    def _update_gc_length(self, global_catalog):
        new_length = len(global_catalog._catalog.paths)
        global_catalog._catalog._length.set(new_length)

    @transact
    def _update_gc_from_worker(self, dmd, catalog_id):
        worker_catalog = getattr(dmd.zport, catalog_id, None)
        global_catalog = dmd.global_catalog
        if worker_catalog:
            ...
            dmd.zport._delObject(catalog_id)
            self._update_gc_length(global_catalog)
            return len(worker_catalog._catalog.paths)
        return 0
    ...
    def _process_catalog_service(self, source_target, worker_count, buffer_size, input_queue_size, processed_queue_size, resume, permissions_only, print_progress):
        ...
        for i in range(worker_count):
            ...
            self.count_committed += self._update_gc_from_worker(dmd, catalog_id)
