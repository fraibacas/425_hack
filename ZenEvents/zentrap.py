...
class SnmpTrapPreferences(CaptureReplay):
    ...
    def buildOptions(self, parser):
        ...
        parser.add_option('--trapfilterfile',
                            dest='trapFilterFile',
                            type='string',
                            help=("File that contains trap oids to keep, should be in $ZENHOME/etc. If"
                                  " hubenabledtrapfilter is set, the filter will only be enabled if zenhub "
                                  " sends an event throttle or pause message"),
                            default=None)
        parser.add_option('--hubenabledfilter', dest='hubEnabledFilter',
                           action='store_true', default=False,
                           help='Trap filter enabled by hub to throttle events')
...
class TrapTask(BaseTask, CaptureReplay):
    ...
    def decodeSnmpv1(self, addr, pdu):
        ...
        # pull src address from the pdu in case customer sends snmptraps 
        # from the localhost. ZEN-18545
        new_addr = '.'.join(map(str, [pdu.agent_addr[i] for i in range(4)]))
        result["device"] = addr[0] if addr[0] != "127.0.0.1" else new_addr
