...
class RenderServer(RRDToolItem):
    ...
    def render(self, gopts=None, start=None, end=None, drange=None,
        ...
        @param getImage: (ignored, deprecated) return the graph or a script location
        ...
        timestampsAppended = 0
        if start:
            timestampsAppended = timestampsAppended + 1
        ...
        if not graph:
            ...
            if remoteHost or remoteUrl:
                ...
                now = int(time.time())
                if end and start:
                    """ Usually end and start passed from UI looks like:
                        end=now-86400s,
                        start=end-129600s.
                        in other cases, inputs look like:
                        end=now--36826s # notice double --
                        end=1416203999  # timestamp, not relative to now
                    """
                    def getEpochTime(timearg, relativeTo):
                        if timearg.isdigit():
                            return int(timearg)
                        return relativeTo - int(timearg[4:-1])
                    end = getEpochTime(end, now)
                    start = getEpochTime(start, end)
                else:
                    end = now - 300
                    start = end - drange
                gopts.insert(0, 'COMMENT:%s\\c' %
                                (comment or self._build_comment(start, end)))
                if timestampsAppended > 0:
                    gopts = gopts[:-timestampsAppended]
    ...
    def _build_comment(self, start, end, comment_format='%s\t\tto\t\t%s',
                        datetime_format='%Y-%m-%d %H\:%M\:%S %Z'):
        start_dt = datetime.datetime.fromtimestamp(start, LOCAL_TZ)
        end_dt = datetime.datetime.fromtimestamp(end, LOCAL_TZ)
        return comment_format % (start_dt.strftime(datetime_format),
