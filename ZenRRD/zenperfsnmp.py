...
from Products.ZenUtils.LocalPingDownMessenger import LocalPingDownMessenger
...
class SnmpPerformanceCollectionPreferences(object):
    ...
    def buildOptions(self, parser):
        ...
        parser.add_option('--uselocalpingmessenger',
                  dest='uselocalpingmessenger',
                  action="store_true",
                  default=False,
                  help="Use this for snmp to use local ping messenger to know if device is responded to zenping or not.")
...
class SnmpPerformanceCollectionTask(BaseTask):
    ...
    def __init__(self,
        ...
        if self._preferences.options.uselocalpingmessenger:
            self._daemon = zope.component.getUtility(ICollector)
            self._localMessenger = LocalPingDownMessenger(self._daemon.options.monitor)
    ...
    def _doCollectOids(self, ignored):
        ...
        try:
            ...
            elif self._preferences.options.uselocalpingmessenger and \
                    not self._localMessenger.isIpDown(self._manageIp) or \
                    not self._responseReceived:            
