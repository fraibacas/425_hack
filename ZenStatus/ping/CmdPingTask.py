...
class CmdPingTask(ZenStatus.PingTask):
    ...
    def _pingIp(self):
        ...
        exitCode = -1
        timestamp = None
        # we need to increase the packet size to atleast 16 because 64bit version of ping
        # does not return RTT data if packet lenght is < 16
        #
        data_length = self._daemon.options.dataLength if self._daemon.options.dataLength > 16 else 16
