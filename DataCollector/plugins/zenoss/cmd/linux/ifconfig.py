...
def _dropInterfaceSpeed(relMap):
    """
...
def parseDmesg(dmesg, relMap):
    ...
    """
    relMap = _dropInterfaceSpeed(relMap)
    for line in dmesg.splitlines():
        speedMatch = speedPattern.search(line)
        if speedMatch:
            for objMap in relMap.maps:
                if objMap.interfaceName == 'lo':
                      objMap.interfaceName = ' lo '             
...
def parseSysNet(sysNet, relMap):
    """
    parse contents of /sys/class/net/* to get interface speed.
    """
    relMap = _dropInterfaceSpeed(relMap)
    sysNetDict = {i.split(':')[0]:i.split(':')[1] for i in sysNet.split('\n')}
    for objMap in relMap.maps:
        try:
            objMap.speed = int(sysNetDict[objMap.interfaceName]) * 1e6
...
class ifconfig(LinuxCommandPlugin):
    ...
    command = 'export PATH=$PATH:/sbin:/usr/sbin; \
               if which ifconfig >/dev/null 2>&1; then \
                   ...
                   echo __COMMAND__ && for path in /sys/class/net/*;\
                   do [ -d "${path}" ] || continue;\
                   dirname="$(basename "${path}")";\
                   cat ${path}/speed > /dev/null 2>&1;\
                   if [[ 0 == $? ]] ; then echo ${dirname}:$(cat ${path}/speed);\
                   fi; done'
    compname = "os"
    relname = "interfaces"
    ...
    def process(self, device, results, log):
        ...
        self.log = log
        ifconfig, dmesg, sysNet = results.split('__COMMAND__')
        sysNet = sysNet.strip()
        if '###' in ifconfig:
        ...
        if sysNet:
            relMap = parseSysNet(sysNet, relMap)
        else:
