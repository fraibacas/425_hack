...
class SshClient(CollectorClient.CollectorClient):
    ...
    def __init__(self, hostname, ip, port=22, plugins=[], options=None,
        ...
        self.collectionsLimit = 0
        try:
            self.collectionsLimit = options.collections_limit
    ...
    def channelClosed(self):
        ...
        if self.commandsFinished():
            if self.collectionsLimit:
                self.collectionCnt += 1
                if self.collectionCnt >= self.collectionsLimit:
                    self.isLoseConnection = True
